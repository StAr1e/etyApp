...
// Add Telegram typing to window
declare global {
  interface Window {
    TelegramApp?: { WebApp?: TelegramWebApp };
  }
}

export const ProfileModal: React.FC<ProfileModalProps> = ({ stats, onClose }) => {
  const levelInfo: LevelInfo = getLevelInfo(stats.xp);

  const progress = Math.min(
    100,
    Math.max(
      0,
      ((stats.xp - levelInfo.minXP) / (levelInfo.nextLevelXP - levelInfo.minXP)) * 100
    )
  );
      ...
      <div className="bg-tg-bg w-full max-w-md md:max-w-lg rounded-t-[2rem] md:rounded-3xl shadow-2xl animate-in slide-in-from-bottom duration-300 relative border-t md:border border-white/20 dark:border-white/5 max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="px-8 pt-8 pb-4">
          ...
          <p className="text-center text-xs text-tg-hint mt-2">
            {Math.max(levelInfo.nextLevelXP - stats.xp, 0)} XP to next rank
          </p>
        </div>
